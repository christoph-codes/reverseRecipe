...
const queryFromNameList = gql`
	...
	query queryRecipesFromNameList($nameList: [string!]) {
		queryIRecipesFromNameList(input: $nameList) {
			...fullRecipeQuery
		}
	}
`;
...
const queryFromIdList = gql`
	...
	query queryRecipesFromNameList($idList: [ID!]) {
		queryRecipesFromIdList(input: $idList) {
			...fullRecipeQuery
		}
	}
`;
...
const queryFromIngredients = gql`
	...
	query queryRecipesFromIngredients($ingredientList: [ID!]) {
		queryRecipesFromIngredients(input: $ingredientList) {
			...fullRecipeQuery
		}
	}
`;
...
export function queryRecipesFromNameList(nameList) {
	...
	if (error) {
		throw new Error(
			`Failed to fetch recipe names ${nameList.join(', ')}: ${
				error.message
			}`
		);
	}
...
export function queryRecipesFromIdList(idList) {
	...
	if (error) {
		throw new Error(
			`Failed to fetch recipe id's ${idList.join(', ')}: ${error.message}`
		);
	}
