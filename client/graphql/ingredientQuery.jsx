...
const queryFromNameList = gql`
	...
	query queryIngredientsFromNameList($nameList: [string!]) {
		queryIngredientsFromNameList(input: $nameList) {
			...fullIngredientQuery
		}
	}
`;
...
const queryFromIdList = gql`
	...
	query queryIngredientsFromNameList($idList: [ID!]) {
		queryIngredientsFromIdList(input: $idList) {
			...fullIngredientQuery
		}
	}
`;
...
export function queryIngredientsFromNameList(nameList) {
	...
	if (error) {
		throw new Error(
			`Failed to fetch ingredient names ${nameList.join(', ')}: ${
				error.message
			}`
		);
	}

	const ingredients = data?.queryIngredientsFromNameList ?? [
		'no ingredient found',
	];
	return {
		ingredients,
...
export function queryIngredientById(id) {
	...
	if (error) {
		throw new Error(
			`Failed to fetch ingredient id ${id}: ${error.message}`
		);
	}
...
export function queryIngredientsFromIdList(idList) {
	...
	if (error) {
		throw new Error(
			`Failed to fetch ingredient id's ${idList.join(', ')}: ${
				error.message
			}`
		);
	}

	const ingredients = data?.queryIngredientsFromIdList ?? [
